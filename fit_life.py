"""Фитнес-трекер FitLife - сбор данных пользователя и расчёт ИМТ и нормы
воды.
"""
...
WATER_PER_KG = 0.030  # 30 мл на кг веса
    ...
    while True:
        try:
            ...
            if MIN_AGE <= age <= MAX_AGE:
                return age
            print(
                f"Пожалуйста, введите возраст от {MIN_AGE} до {MAX_AGE} "
                "лет.\nПопробуй ещё раз:",
            )
        except ValueError:
            print(
                "Нужно ввести целое число (например, 22). Попробуй ещё раз:",
            )
...
def get_valid_name():
    ...
    while True:
        ...
        if any(ch.isdigit() for ch in name):
            print(
                "Имя не должно содержать цифр. Введи имя буквами (русскими "
                "или английскими).",
            )
        elif len(name) == 0:
    ...
    while True:
        try:
            ...
            if MIN_WEIGHT <= weight <= MAX_WEIGHT:
                return weight
            print(
                f"Пожалуйста, введи реальный вес (от {MIN_WEIGHT} до "
                f"{MAX_WEIGHT} кг).\nПопробуй ещё раз:",
            )
        except ValueError:
            print(
                "Ошибка! Нужно ввести число (например, 70.5). Попробуй ещё "
                "раз:",
            )
    ...
    while True:
        try:
            ...
            if MIN_HEIGHT <= height <= MAX_HEIGHT:
                return height
            print(
                f"Пожалуйста, введи рост в метрах (от {MIN_HEIGHT} до "
                f"{MAX_HEIGHT}).\nНапример, 1.75. Попробуй ещё раз:",
            )
        except ValueError:
            print(
                "Ошибка! Нужно ввести число (например, 1.75). Попробуй ещё "
                "раз:",
            )
...
if __name__ == "__main__":
    print(
        "Привет! Я твой личный фитнес-трекер - FitLife!\n- а как зовут "
        "тебя?",
    )
    user_name = get_valid_name()

    print(
        f"{user_name}, очень приятно!\nТеперь подскажи, сколько тебе лет?",
    )
    ...
    print(
        "Прекрасно! Осталось узнать твой рост. Укажи его в метрах "
        "(например, 1.75):",
    )
